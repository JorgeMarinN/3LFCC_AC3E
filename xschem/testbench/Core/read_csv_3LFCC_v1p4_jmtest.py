...
df=pd.read_fwf('3LFCC_v1p5_jmtest.txt')
df.to_csv('3LFCC_v1p5_jmtest.csv', index=False)
data = pd.read_csv("3LFCC_v1p5_jmtest.csv").values
...
r = 22
vout = np.mean(y_out[peaks[0]:peaks[24]])
iin = np.mean(y_in[peaks[0]:peaks[24]])
ils = np.mean(y_in_LS[peaks[0]:peaks[24]])
iout = np.mean(data_iout[peaks[0]:peaks[24]])
#iout = vout/r
pout = vout*iout
pin = 5*iin
...
plt.plot(x_in[peaks[0]:peaks[24]],y_in[peaks[0]:peaks[24]])
plt.show()

plt.plot(x_in_LS[peaks[0]:peaks[24]],y_in_LS[peaks[0]:peaks[24]])
plt.show()

plt.plot(x_out[peaks[0]:peaks[24]],y_out[peaks[0]:peaks[24]])
plt.show()
